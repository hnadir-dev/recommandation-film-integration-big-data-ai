...
from pyspark.sql.functions import from_json, col, lit, concat,to_date,to_timestamp
...
write_movies_query = (sel_movies.writeStream \
    .outputMode("append") \
    .option("checkpointLocation", "/tmp/elasticsearch_movies/") \
    .foreachBatch(lambda batch_df, batch_id: write_to_elasticsearch(batch_df,'rec-movies-index')) \
    .start())

write_users_query = (sel_users.writeStream \
    .outputMode("append") \
    .option("checkpointLocation", "/tmp/elasticsearch_users/") \
    .foreachBatch(lambda batch_df, batch_id: write_to_elasticsearch(batch_df,'rec-users-index')) \
    .start())

write_ratings_query = (sel_ratings.writeStream \
    .outputMode("append") \
    .option("checkpointLocation", "/tmp/elasticsearch_lv3/") \
    .foreachBatch(lambda batch_df, batch_id: write_to_elasticsearch(batch_df,'rec-ratings-index')) \
    .start())
